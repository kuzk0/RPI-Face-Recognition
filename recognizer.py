...
FRAME_SKIP = 1          # Process every 3rd frame for speed
CONFIDENCE_THRESHOLD = 30.0  # % — минимальная уверенность для признания совпадения

# Время в секундах, в течение которого прямоугольник остаётся полупрозрачным
FADE_TIMEOUT_SEC = 0.5  
# Скорость затухания (меньше = медленнее)
FADE_SPEED = 2         # Примерно: за N кадров прозрачность упадёт на 1
...
# Расстояние в диапазоне [0, 2], где 0 — идеальное совпадение
def convert_distance_to_confidence(distance: float) -> float:
    # Ограничим макс. возможное расстояние значением 2.5 (для стабильности)
    max_dist = 2.5
    if distance <= 0:
        return 100.0
    if distance >= max_dist:
        return 0.0
    # Линейная интерполяция: расстояние от 0 до max_dist → уверенность от 100% до 0%
...
def get_text_color_for_background(rgb):
    """
    Determines whether to use black or white text based on background brightness.
    
    Args:
        rgb: A tuple of (R, G, B) values in the range [0, 255].
        
    Returns:
        A tuple representing the color for text (B, G, R).
    """
    # Calculate relative luminance using standard formula
    r, g, b = rgb[0], rgb[1], rgb[2]
    luminance = 0.299*r + 0.587*g + 0.114*b
    
    # If luminance > 128 (medium bright), use black text; otherwise white
    if luminance > 128:
...
def main():
    ...
    # Dictionary to store colors for each person
    person_colors = {}
    ...
    try:
        while True:
            ...
            frame_count += 1
            process_frame = (frame_count % FRAME_SKIP == 0) or (frame_count < 5)
            ...
            if process_frame:
                try:
                    ...
                    for det in current_detections:
                        ...
                        # Assign a random color if this person hasn't been seen before
                        ...
                        if best_key_to_replace is not None:
                            # Preserve the existing color when updating entry
                            new_cache[best_key_to_replace] = new_entry
                            new_cache[best_key_to_replace]['color'] = face_cache.get(best_key_to_replace, {}).get('color', person_colors[new_entry['name']])
                        else:
                    ...
                    # Merge with old cache for fade-out effect
            ...
            # --- Drawing logic with confidence and person-specific colors ---
            current_time = time.time()
            for key, cached_data in list(face_cache.items()):
                ...
                # Calculate alpha (transparency) for fade-out effect
                ...
                # Convert base color to BGR with alpha (opacity)
                ...
                bg_rgb = [base_color[2], base_color[1], base_color[0]]  # Convert BGR to RGB for luminance calculation
                ...
                if time_since_detection <= fade_total_time:
                    # Fill label background with person's color and alpha
                ...
                # Put text with appropriate color
                cv2.putText(frame,
                           label,
                           (x, y - 15),
